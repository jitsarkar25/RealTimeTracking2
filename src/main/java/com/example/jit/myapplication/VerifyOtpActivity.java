...
public class VerifyOtpActivity extends AppCompatActivity {
    ...
    public void verifyotp(View v) {
        final SharedPreferences sharedPreferences = getSharedPreferences("otpcheck",MODE_PRIVATE);
        String phone = sharedPreferences.getString("phone","");
        String otp =ot.getText().toString();
        if(otp.equals("") || otp.length()!=4)
        {
            Toast.makeText(getApplicationContext(),"Enter Proper OTP",Toast.LENGTH_SHORT).show();
        }
        else {
